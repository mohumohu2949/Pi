...
# 1000000倍の値をスコアに入れるために一旦移動
    ...
    execute store result storage pi: Calc.Count int 1 run scoreboard players get *count _
# スコアホルダーに格納
    execute store result score *a _ run data get storage pi: Calc.In 4000
    execute store result score *b _ run data get storage pi: Calc.Count 1
# 計算結果を代入
...
# reset
    ...
    data remove storage pi: Calc.Count